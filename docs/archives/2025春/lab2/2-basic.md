...
实验框架中日志不是实时刷盘的，日志会先记录在内存中，只有事务提交和脏页淘汰时会调用 LogManager::Flush 函数将部分日志刷盘。因此，在数据库故障时可能会有部分没有刷盘的日志丢失，如课程 PPT 案例说明中 LSN 为 14 的日志，从而导致处理过程与课程 PPT 中的方式有所不同。此外，实验框架中并不会将 flush 操作记录在日志中，因此脏页表内容也与课程案例说明中的例子有差别，在对照课程 PPT 进行调试时需要注意这一点。
